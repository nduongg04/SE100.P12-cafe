...
function StarRating({ rating, setRating, form }: { rating: number; setRating: (value: number) => void; form: any }) {
  return (
    <div className="mb-6 flex justify-center">
      {[1, 2, 3, 4, 5].map((star) => (
        <Star
          key={star}
          className={`h-8 w-8 cursor-pointer ${star <= (form.watch("starNumber") || rating) ? "fill-[#00B074] text-[#00B074]" : "text-gray-300"
            }`}
          onClick={() => {
            setRating(star);
...
function DishRating({ index, form, products, ratedProducts, setRatedProducts }: { index: number; form: any; products: Product[]; ratedProducts: Product[]; setRatedProducts: React.Dispatch<React.SetStateAction<Product[]>> }) {
  return (
          ...
          <FormItem>
            ...
            <Select
              onValueChange={(value) => {
                ...
                if (selectedProduct) {
                  setRatedProducts((prev) => [...prev, selectedProduct]);
                }
              }}
            ...
            >
              ...
              <SelectContent>
                {products
                  .filter(
                    (product) =>
                      !ratedProducts.some((ratedProduct) => ratedProduct.productID === product.productID) ||
            ...
            <FormControl>
              <div className="flex items-center">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star
                    key={star}
                    className={`h-6 w-6 cursor-pointer ${star <= field.value ? "fill-[#00B074] text-[#00B074]" : "text-gray-300"
                      }`}
                    onClick={() => form.setValue(`dishes.${index}.rating`, star)}
                  />
                ))}
              </div>
            </FormControl>
    ...
    try {
      await createFeedback({
        ...
        listProdFb: values.dishes
          ...
          .map((dish) => {
            const product = products.find((product) => product.productName === dish.name);
            if (!product) {
        ...
        <div className="rounded-lg bg-white p-8 shadow">
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              <StarRating rating={rating} setRating={setRating} form={form} />
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  form.setValue(`dishes.${dishRatings.length}`, { name: "", rating: 0 });
                }}
              >
